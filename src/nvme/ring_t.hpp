...
public:
  typedef T* T_ptr;
  typedef atomic<uint64_t> valid_t;
...
public:
  ...
  inline void incr_valid(size_t idx, uint64_t amount = 1) {
    valid[idx].fetch_add(amount, DEFAULT_MEMORY_ORDER);
  }

  inline valid_t get_valid(size_t idx) {
    return valid[idx].load(DEFAULT_MEMORY_ORDER);
  }
  inline bool is_valid(size_t idx) {
    return valid[idx].load(DEFAULT_MEMORY_ORDER) >= VALID_THRESHOLD;
  }

  inline valid_t* get_valid_ptr(size_t idx) {
    return &valid[idx];
  }
  ...
  inline bool is_tail_valid() {
    return valid[tail].load(DEFAULT_MEMORY_ORDER) >= VALID_THRESHOLD;
  }
  ...
  inline size_t advance_valid() {
    size_t cnt = 0;
    while (valid[head_valid].load(DEFAULT_MEMORY_ORDER) >= VALID_THRESHOLD) {
      cnt++;
  ...
  inline size_t release_valid() {
    size_t count = 0;
    // Advance tail
    while (valid[tail].load(DEFAULT_MEMORY_ORDER) >= VALID_THRESHOLD) {
      valid[tail] = 0;
      count++;
      tail = incr(tail);
      cur_size--;
