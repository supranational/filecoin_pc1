...
class coordinator_t {
  ...
  atomic<size_t> coord_done_count[COORD_BATCH_COUNT];
#ifdef MUTEX_THREAD_SYNC
  mutex          coord_ready_mtx[COORD_BATCH_COUNT];
#endif
  ...
  node_t* coord_ring_ptrs[COORD_BATCH_COUNT][COORD_BATCH_SIZE * PARENT_COUNT];

public:
  coordinator_t(atomic<bool>&               _terminator,
                ...
                vector<replica_id_buffer_t> replica_id_buffers // One per sector
                ) :
    ...
    coord_next_valid(_node_start)    
  {
    ...
    assert (replica_id_buffers.size() == topology.num_hashers);
  ...
  void hash(node_id_t node, coord_node_t* parent_nodes,
            node_t** local_ptrs, node_t* hash_out,
    ...
    for (size_t i = 0; i < PARENT_COUNT; i++) {
      if (local_ptrs[i] != nullptr) {
        ptr_batch.batch[i].ptr = (node_t*)&local_ptrs[i]->sectors[offset_id];
      } else {
        ptr_batch.batch[i].ptr = (node_t*)&parent_nodes[i].sectors[thread_id * NODES_PER_HASHER];
      }
    }
    
    node_ptr_t* cur_parent_ptr  = &ptr_batch.batch[0].ptr;
    ...
    //                  offset_id, thread_id, node.id(), node.node(), j);
    //         //size_t node_idx = batch->batch[j].node % NODES_PER_PAGE;
    ...
    for (size_t j = 0; j < PARENT_PTR_BATCH_SIZE; j++) {
      __builtin_prefetch(&ptr_batch.batch[j].ptr->sectors[0][0], 0, 3);
      __builtin_prefetch(&ptr_batch.batch[j].ptr->sectors[0 + 1][0], 0, 3);
    }
    ...
    // Hash the node
    sha_ext_mbx2(hash_out->sectors[offset_id],
    ...
    if (offset_id == 18 &&
      ...
      for (size_t j = 0; j < PARENT_PTR_BATCH_SIZE; j++) {
        ...
        if (ptr_batch.batch[j].ptr == nullptr) {
          ...
          snprintf(prefix, 64, "H %ld N %ld %8lx.%2d P %02ld ",
                   offset_id, thread_id, node.id(), node.node(), j);
          //size_t node_idx = batch->batch[j].node % NODES_PER_PAGE;
          //print_node((node_t*)ptr_batch.batch[j].ptr->sectors[0], 2, prefix);
      ...
      printf("H %ld T %ld node %lx hasher out %p: ",
             offset_id, thread_id, node.id(), hash_out->sectors[offset_id]);
      print_digest_reorder(hash_out->sectors[offset_id]);
  ...
  __attribute__ ((noinline)) void run_thread(size_t thr_count, size_t sector) {
    ...
    while (thr_node < node_stop) {
      ...
      while (thr_node.id() < valid_nodes) {
        ...
        node_t** local_ptr_batch = coord_ring_ptrs[coord_idx];
        ...
        for (size_t i = 0; i < COORD_BATCH_SIZE; i++) {
          size_t node_in_page = thr_node.node() % NODES_PER_PAGE;
          coord_node_t* parent_nodes = &parent_batch[i * PARENT_COUNT];
          node_t* hash_out = &system.node_buffer.get_entry(node_idx)->batch[0].nodes[node_in_page];
          ...
          thr_node++;
          if (thr_node.node() % NODES_PER_PAGE == 0) {
...
// #endif
  ...
  int run() {
    ...
    timestamp_t layer_start = chrono::high_resolution_clock::now();
    ...
    node_t** coord_ring_ptrs_batch = coord_ring_ptrs[coord_batch_idx];
    ...
    while(node < node_stop) {
      ...
      while (!done && cur_valid) {
        ...
        for (size_t j = 0; j < PARENT_COUNT; j++) {
          ...
          node_t* src = ptr_batch->batch[j].ptr;
...
inline void coordinator_clear_stats(coordinator_t *coordinator) {
  coordinator->clear_stats();
}
inline void coordinator_snapshot(coordinator_t *coordinator) {
  coordinator->snapshot();
}
inline void coordinator_print(coordinator_t *coordinator) {
  coordinator->print();
}
