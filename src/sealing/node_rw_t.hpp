...
inline void nvme_node_indexes(size_t node, size_t &ctrl_id, size_t &block_on_controller) {
  size_t block = node / NODES_PER_PAGE;
  block_on_controller = block / NUM_CONTROLLERS;
  ctrl_id = block - block_on_controller * NUM_CONTROLLERS;
}

// Process read/write IO requests
// Templated with a batch_t specialization
template<class B>
class node_rw_t {
...
public:
  node_rw_t(atomic<bool>& _terminator,
...
public:
  // Process IO requests
  int process() {
    // Print controllers structs
    printf("Processing IOs for controllers\n");
    for (size_t ctrl_id = 0; ctrl_id < controllers.size(); ctrl_id++) {
      printf("  %ld - %s\n", ctrl_id, controllers[ctrl_id].get_name().c_str());
    }
    ...
    while (!terminator) {
      ...
      for (size_t ctrl_id = 1; ctrl_id < controllers.size(); ctrl_id++) {
        ...
        min_free = min(min_free, free_slots);
      }
      ...
      size_t num_batches = min(min_free / B::BATCH_SIZE, available);
      ...
      for (size_t i = 0; i < num_batches; i++) {
        ...
        for (size_t j = 0; j < B::BATCH_SIZE; j++) {
          ...
          nvme_node_indexes(req->node, ctrl_id, strided_block);
...
template<class B>
inline void rw_clear_stats(node_rw_t<B> *rw) {
  rw->clear_stats();
}
template<class B>
inline void rw_snapshot(node_rw_t<B> *rw) {
  rw->snapshot();
}
template<class B>
inline void rw_print(node_rw_t<B> *rw) {
  rw->print();
}
