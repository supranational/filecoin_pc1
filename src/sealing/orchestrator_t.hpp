...
public:
  orchestrator_t(atomic<bool> &_terminator,
                 system_buffers_t& _system,
  ...
  size_t read_batch(node_id_t min_hash_node, node_buffer_iterator_t& tail_node_write,
    ...
    const size_t MAX_READ_BATCH = NODES_PER_PAGE * 2;
    ...
    const size_t BATCH_INCREMENT = NODES_PER_PAGE;
    ...
    if (node_buffer_free * NODES_PER_PAGE < MAX_READ_BATCH) {
      node_buffer_full++;
    ...
    size_t min_free = min(parent_read_fifo_free, node_buffer_free * NODES_PER_PAGE);
    min_free = min(min_free, parent_buffer_free);
    size_t max_batch = min(min_free, remaining_nodes);
    max_batch = min(max_batch, MAX_READ_BATCH);
    ...
    size_t node_buffer_count = system.node_buffer.size() * NODES_PER_PAGE - max_batch;
    node_id_t cache_min_node = node - node_buffer_count;
    node_id_t cache_min_cacheable_node =
      min(node_id_t(cache_min_node + cache_skid_size), // Keep space from the tail
    ...
    for (size_t i = 0; i < max_batch;
      ...
      node_id_t node = parent_iter.get_node(); // node we're reading parents for
      size_t node_in_node_buffer_page = node.node() % NODES_PER_PAGE;
      node_t* cur_node_buffer =
        &node_batch_ptrs[i / NODES_PER_PAGE]->batch[0].nodes[node_in_node_buffer_page];
      ...
      page_batch_t* page_batch = page_batch_ptrs[i];
      // Batch of IO requests going to disks
      page_io_batch_t* io_batch = &system.parent_buffer_io[parent_buffer_id];
      // Batch of parent pointers - aligns with parent buffers
      parent_ptr_batch_t* ptr_batch = &system.parent_ptrs[parent_buffer_id];
      parent_ptr_sync_batch_t* ptr_sync_batch = &system.parent_ptr_syncs[parent_buffer_id];
        ...
        if (use_cache) {
          ...
          size_t node_buffer_entry =
            system.node_buffer.add(system.node_buffer.get_tail(),
                                   parent_node_cache_offset.id() / NODES_PER_PAGE);
          ...
          node_t* tmp_node = &tmp_batch->batch[0].nodes[node_in_page];
          ...
          // and the zeroeth entry references the previous hashed node.
          ptr_batch->batch[j + 1].ptr = &page_batch->batch[j].nodes[node_in_page];
          ptr_sync_batch->batch[j + 1].node_buffer_idx = parent_ptr_sync_t::NOT_NODE_BUFFER;
  ...
  size_t write_nodes(node_buffer_iterator_t& head_node_write, node_id_t min_node_hash) {
    size_t head_page_write = head_node_write.abs().id() / NODES_PER_PAGE;
    size_t min_page_hash = min_node_hash.id() / NODES_PER_PAGE;
    ...
    // node_t *node = &system.node_buffer.get_entry(head_node_write.idx())->batch[0].nodes[0];
  ...
  __attribute__ ((noinline)) int process(bool print_timing = true) {
    ...
    while (node < node_stop
      ...
      while (actions_remaining-- > 0 && head_node_write.abs() < min_node_hash) {
        ...
        outstanding_writes += writes_issued / NODES_PER_PAGE;
        writes_issued += writes_issued / NODES_PER_PAGE;
        if (writes_issued > 0) {
#ifdef PRINT_STALLS
      ...
      while (actions_remaining-- > 0 && tail_node_parents < min_node_hash) { 
        parent_ptr_sync_batch_t* ptr_sync_batch =
      ...
      while (actions_remaining-- > 0 &&
        ...
        outstanding_writes--;
        tail_node_write += NODES_PER_PAGE;
#ifdef PRINT_STALLS
      ...
      while (actions_remaining-- > 0 &&
        ...
        if ((tail_node.node() % NODES_PER_PAGE) == NODES_PER_PAGE - 1) {
      ...
      if (parent_iter.get_node().id() - tail_node.id() >=
          NODE_BUFFER_BATCHES * NODES_PER_PAGE) {
