...
public:
  ...
  atomic<node_id_t> coordinator_node[NUM_HASHING_COORDS];

  // Coordinator pointers
  coordinator_t* coordinators[NUM_HASHING_COORDS];

  // Pointer to the parent reader
  node_rw_t<page_io_batch_t>* parent_reader;
...
public:
  ...
  //int init(size_t num_disks, node_rw_t* rw) {
  int init(size_t num_disks) {
    ...
    for (size_t i = 0; i < PARENT_BUFFER_BATCHES; i++) {
      for (size_t j = 0; j < page_batch_t::BATCH_SIZE; j++) {
        ...
        page_t* parent_buffer_page = &parent_buffer.get_entry(i)->batch[j];
    ...
    for (size_t i = 0; i < NODE_BUFFER_BATCHES; i++) {
      for (size_t j = 0; j < node_io_batch_t::BATCH_SIZE; j++) {
        ...
        page_t* node_buffer_page = &node_buffer.get_entry(i)->batch[j];
