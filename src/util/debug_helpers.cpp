...
void print_parameters() {
  ...
  std::cout << "COMM_D_DEPTH          "<< COMM_D_DEPTH << std::endl;
  std::cout << "PARALLEL_SECTORS      "<< PARALLEL_SECTORS << std::endl;
  std::cout << "NODES_PER_HASHER      "<< NODES_PER_HASHER << std::endl;
  std::cout << "HASHERS_PER_CORE      "<< HASHERS_PER_CORE << std::endl;
...
void print_node(node_t *buf, size_t lines, const char *prefix, bool reverse) {
  _print_buf((uint8_t*)buf, lines, prefix, reverse, PARALLEL_SECTORS);
}
